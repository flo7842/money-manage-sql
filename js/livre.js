require('./db/database')
const {Istmt} = require('./db/database')
...
loadTablesLines = function (date) {
    ...
    Istmt.findAll({ where: { [Op.and]: [{ date_entry: {[Op.gte]: minDate} }, {date_entry: {[Op.lte]: maxDate} } ] }}).then(async (docs, err) =>{
        //console.log('all data here', err)
        ...
        if(docs.length !== 0){
            ...
            docs.forEach((el, i) => {
                ...
                let dateFR = convertDate(el.date_entry)
                ...
                btnRemove.addEventListener('click', async () => {
                    if(confirm('êtes vous sur de vouloir supprimer cette ligne ?')){
                        ...
                        await Istmt.destroy({ where: {id: el.id }}, {}, function (err, numRemoved) {
                            if(err != null){
                                console.log(err)
                            }
