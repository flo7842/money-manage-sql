const {Employees, Company} = require('./db/database')
...
openPayslipModalBtn.addEventListener('click', async function(e){
  ...
  let nowDateFr = moment().format('DD/MM/YYYY')
    ...
    let doc = await Employees.findOne({ firstname: firstNameEmploye, lastname: lastNameEmploye })
    if(nowDateFr > startOfMonth && nowDateFr < endOfMonth){
      
      if(doc.paid_holidays < 30){
       
        await Employees.updateOne({_id: doc._id}, 
        { 
...
$('#selectEmployee').on('change', function (e) {
  ...
  if(slicedOpts.length > 0){
    ...
    Employees.findAll({where: { [Op.and]: [{ firstname: slicedOpts[2], lastname: slicedOpts[1] }]}}).then((docs) => {
      console.log('Docs',docs)
      
      libelleEmpl.innerHTML = docs[0].civility + ' ' + docs[0].lastname + ' ' + docs[0].firstname
      addressLabel.innerHTML = docs[0].address_number + ' ' + docs[0].address
      addressNumberLabel.innerHTML = docs[0].postal_code + ' ' +docs[0].city
      
      socialLabel.innerHTML = docs[0].social_number
      qualificationLabel.innerHTML = docs[0].qualification
      registrationLabel.innerHTML = docs[0].registration_number
      job.innerHTML = docs[0].occupation
      entryDate.innerHTML = convertDate(docs[0].entry_date)
      $('#grossSalary').val(docs[0].gross_salary)
